...
struct SkeletonFallsGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://skelfallblock.pro/client")! }

    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            SkeletonFallsEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}

#Preview {
    SkeletonFallsGameInitialView()
}
