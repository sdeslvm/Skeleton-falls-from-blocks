...
struct SkeletonFallsWebViewBox: UIViewControllerRepresentable {
    // MARK: - Свойства

    @ObservedObject var loader: SkeletonFallsWebLoader
    var defaultOrientations: UIInterfaceOrientationMask = .portrait

    // MARK: - Координатор

    func makeCoordinator() -> SkeletonFallsWebCoordinator {
        SkeletonFallsWebCoordinator { [weak loader] status in
    ...
    func makeUIViewController(context: Context) -> SkeletonFallsBaseWebViewController {
        let configuration = createWebViewConfiguration()
        return SkeletonFallsBaseWebViewController(
    ...
    func updateUIViewController(_ uiViewController: SkeletonFallsBaseWebViewController, context: Context) {
