...
protocol WebLoadable: AnyObject {
    var state: SkeletonFallsWebStatus { get set }
...
final class SkeletonFallsWebLoader: NSObject, ObservableObject, WebLoadable, ProgressMonitoring {
    // MARK: - Свойства

    @Published var state: SkeletonFallsWebStatus = .standby
    ...
    func observeProgression() {
        progressPublisher
            ...
            .sink { [weak self] progress in
                ...
                let newState: SkeletonFallsWebStatus = progress < 1.0 ? .progressing(progress: progress) : .finished
...
extension SkeletonFallsWebLoader: WKNavigationDelegate {
...
extension SkeletonFallsWebLoader {
    ...
    func publish(_ newState: SkeletonFallsWebStatus) {
...
private extension SkeletonFallsWebLoader {
    func updateState(_ newState: SkeletonFallsWebStatus) {
